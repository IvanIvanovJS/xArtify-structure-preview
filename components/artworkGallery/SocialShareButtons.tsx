...
export default function SocialShareButtons({
    ...
    // Share data is used in the sharing functions below
    ...
    const socialPlatforms = [
        ...
        { id: 'copy', name: 'Копирай линк', icon: 'copy' }
    ];
        ...
        <div className={`social-share-buttons ${className}`}>
            <div className="social-share-grid">
                {socialPlatforms.map((platform) => (
                    <button
                        key={platform.id}
                        className={`social-share-button ${platform.id === 'copy' && copied ? 'copied' : ''}`}
                        onClick={() => handleShare(platform.id)}
                        aria-label={`Сподели в ${platform.name}`}
                        title={platform.name}
                    >
                        <div className={`social-share-icon social-share-icon--${platform.icon}`}>
                            {getSocialIcon(platform.icon)}
                        </div>
                        <span className="social-share-label">{platform.name}</span>
                    </button>
                ))}
            </div>
...
function getSocialIcon(iconType: string): React.JSX.Element {
    switch (iconType) {
        case 'facebook':
            return (
