...
import "./styles/other-artists-carousel.css";

interface OtherArtistsCarouselProps {
    excludeArtistId?: string;
    excludePaintingId?: string;
    className?: string;
}

export default function OtherArtistsCarousel({
    excludeArtistId,
    excludePaintingId,
    className = ""
}: OtherArtistsCarouselProps): React.JSX.Element {
    ...
    useEffect(() => {
        const fetchRandomPaintings = async () => {
            try {
                ...
                if (excludeArtistId) {
                    params.append('excludeArtistId', excludeArtistId);
                }
                if (excludePaintingId) {
                    params.append('excludePaintingId', excludePaintingId);
                }
                params.append('limit', '8');

                const response = await fetch(`/api/paintings/random?${params}`);
                if (response.ok) {
    ...
    if (loading) {
        return (
            <div className={`other-artists-carousel ${className}`}>
                <div className="other-artists-carousel-header">
                    <h2 className="other-artists-carousel-title">Произведения от други автори</h2>
                </div>
                <div className="other-artists-carousel-loading">
                    <div className="other-artists-carousel-skeleton">
                        {[...Array(4)].map((_, index) => (
                            <div key={index} className="other-artists-carousel-skeleton-item">
    ...
    return (
        <div className={`other-artists-carousel ${className}`}>
            <div className="other-artists-carousel-header">
                <h2 className="other-artists-carousel-title">Произведения от други автори</h2>
                {paintings.length > 1 && (
                    <div className="other-artists-carousel-navigation">
                        <button
                            className="other-artists-carousel-nav-button"
            ...
            <div
                className="other-artists-carousel-container"
                ref={carouselRef}
            ...
            >
                ...
                >
                    {paintings.map((painting) => (
                        <div key={painting.id} className="other-artists-carousel-item">
                            <ArtworkCard
                                painting={painting}
                                showSold={true}
                            />
            ...
            {/* Dots Indicator */}
            {paintings.length > 1 && (
                <div className="other-artists-carousel-dots">
                    {paintings.map((_, index) => (
                        <button
                            key={index}
                            className={`other-artists-carousel-dot ${currentIndex === index ? 'active' : ''}`}
